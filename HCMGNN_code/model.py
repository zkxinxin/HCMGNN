...
class SemanticEncoder(nn.Module):
    ...
    def forward(self, edata):
        ...
        final_r_vec = torch.zeros([edata.shape[1], self.hidden_size // 2, 2])
    ...
    def forward(self, g, h):
        ...
        for i, meta_path in enumerate(self.meta_paths):
            ...
            for j in range(len(meta_path)):
                edata_list.append(
                    F.embedding(torch.tensor(metapath_instances.iloc[:, j]), h[list(meta_path)[j]]).unsqueeze(1))
    ...
    def get_embed_map(self, features, embed_features, data):
        ...
        for i in range(len(embed_features.keys())):
            for j in range(len(data)):
                if data[j][i] in range(len(embed_features[list(embed_features.keys())[i]])):
                    stack_embedding[list(stack_embedding.keys())[i]].append(
                        embed_features[list(embed_features.keys())[i]][data[j, i].tolist()])
                else:
                    id = int(data[j][i])
                    stack_embedding[list(stack_embedding.keys())[i]].append(
